...
\begin{description}
	...
	\item[ABS.c]: definisce la struttura dati e le operazione necessarie alla sua costruzione e mantenimento.
	...
	\item[Parser.y]: per costruire, in modo coerente anche durante il runtime, la struttura dati in concomitanza alla scansione dei singoli token contiene i puntatori alla struttura dati.
		La generica funzione built-in d'errore \texttt{yyerror} restituisce la posizione raggiunta dal Lexer in quanto durante il parsing è sempre nota la posizione del Lexer ma non dei singoli token ricevuti in input.
		Alla stringa data ad \texttt{yyerror} viene aggiunta una stringa di parametri, contenente la posizione del token a cui l'errore si riferisce. 
